...
const TableCardItem=({orderId,transactionFees,orderDate,orderAmount,index})=>
{
    return (
        <tr >
            ...
            <td className='text-text  p-0 text-sm leading-5 max-sm:text-[8.5px]'>
              <p className={`${tableData.length===(index + 1)?"":"border-b"} border-background-light py-3.5 px-3 max-sm:px-0`}>{formatDate(orderDate)}</p>
              </td>
            ...
            <td className='text-text p-0  text-sm leading-5 max-sm:text-[8.5px]'>
              <p className={`${tableData.length===(index + 1)?"":"border-b"} border-background-light py-3.5 px-3 max-sm:px-0 text-right mr-5`}>₹ {transactionFees}</p>
              </td>
        ...
        <table className=' w-full text-left border-collapse max-sm:table-fixed max-sm:w-full'>
          <thead className='max-sm:w-screen'>
            <tr className="bg-[#F2F2F2] ">
                ...
                <th className='text-text py-2.5 px-3 font-medium bg-[#F2F2F2] text-sm max-sm:text-[8.5px] max-sm:px-0 max-sm:py-1.5 max-sm:w-16'>Order date</th>
                ...
                <th className='text-text py-2.5 px-3 font-medium bg-[#F2F2F2] text-sm max-sm:text-[8.5px] max-sm:px-0 max-sm:py-1.5 max-sm:w-14 text-right mr-5'>Transaction Fees</th>
            </tr>
            </thead>
            <tbody>
            {tableData.map(({orderId,transactionFees,orderDate,orderAmount},index)=>(
                <TableCardItem key={orderId} orderId={281209} transactionFees={transactionFees} orderDate={orderDate} orderAmount={orderAmount} index={index}/>
            ))}
            </tbody>
            </table>
          </div>
